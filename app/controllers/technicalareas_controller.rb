class TechnicalareasController < ApplicationController
  ...
  def show
    @mynotices =  Expertise.where('technicalarea_id = ?', @technicalarea.id)
    @myposts =  PostsTechnicalarea.where('technicalarea_id = ?', @technicalarea.id)
  ...
  private
    ...
    def technicalarea_params
      params.require(:technicalarea).permit(:name, :body)
    end
end
